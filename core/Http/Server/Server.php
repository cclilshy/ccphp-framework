...
class Server
{
                            ...
                            if ($context === '@') {
                                ...
                                while (true) {
                                    if (!isset($_[1])) {
                                        ...
                                        //handler
                                        if (isset(self::$clients[$_[0]])) {
                                            socket_write(self::$clients[$_[0]]['socket'], $_[2]);
                                        }
                                        break;
                                    }
                                }
    ...
    public static function stop(): void
    {
        if ($server = BaseServer::load('HTTP_SERVER')) {
            Process::initialization();
            $handlerProcessIds = $server->info();
            foreach ($handlerProcessIds as $pid) {
                Process::kill($pid);
            }
            ...
            $server->release();
            Console::pdebug('Http Server stop success!');
        }
    }
